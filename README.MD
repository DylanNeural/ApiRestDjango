...
| POST /api/token/ | Login : obtention du couple access + refresh JWT |
| POST /api/token/refresh/ | Rafraîchir le token d’accès à partir du refresh |
| GET /api/auth/me/ | Récupérer le profil de l’utilisateur connecté |
| POST /api/logout/ | Logout : blacklist du refresh token. |
...
| PUT /api/tasks/<id>/ | Mettre à jour complètement une tâche |
| PATCH /api/tasks/<id>/ | Mettre à jour partiellement une tâche |
| DELETE /api/tasks/<id>/ | Supprimer une tâche (d’après ton commentaire : admin only côté API) |
